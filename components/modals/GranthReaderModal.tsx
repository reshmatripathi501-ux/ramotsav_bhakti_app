...
const GranthReaderModal: React.FC<GranthReaderModalProps> = ({ post, onClose }) => {
    return (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div 
                className="bg-orange-50 dark:bg-[#1e0f05] border-2 border-amber-600 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl dark:shadow-amber-900/50 animate-zoom-in"
                onClick={e => e.stopPropagation()}
            >
                <div className="p-4 border-b border-gray-200 dark:border-amber-800 relative">
                    <h2 className="text-xl font-cinzel text-amber-700 dark:text-amber-300 text-center">{post.title}</h2>
